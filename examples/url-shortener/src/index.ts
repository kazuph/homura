...
function parseKvPrefetchKeys(path: string): string[] {
  ...
  const shortenerMatch = path.match(/^\/s\/([^/?#]+)$/);
  if (shortenerMatch && shortenerMatch[1]) {
    try {
      const code = decodeURIComponent(shortenerMatch[1]);
      keys.add(`url:${code}`);
      keys.add(`count:${code}`);
    } catch (e) {
      keys.add(`url:${shortenerMatch[1]}`);
      keys.add(`count:${shortenerMatch[1]}`);
    }
  }
  ...
  if (statsMatch && statsMatch[1]) {
    try {
      ...
      keys.add(`url:${statsMatch[1]}`);
      keys.add(`count:${statsMatch[1]}`);
    }
  }
...
function normalizeLoopOps(rawOps: unknown[]): RubyLoopOp[] {
  ...
  for (let i = 0; i < rawOps.length; i++) {
    ...
    if (explicitKind === undefined && rawOp === '') {
      throw new Error(`Invalid control.ops[${i}].op`);
    }

    if (explicitKind === undefined || explicitKind === 'kv' || rawOp === 'put' || rawOp === 'delete') {
      out.push({
        kind: 'kv',
...
async function executeKvOps(env: Env, ops: Array<KvOp>): Promise<LoopOpResult[]> {
...
// Without this, concurrent requests interleave at await points,
...
export default {
  async fetch(request: Request, env: Env, _ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      const completedKvResults: LoopOpResult[] = [];
      ...
      while (true) {
        ...
        const nextOps = loopOps.filter((op) => op.kind === 'kv');
        ...
        completedKvResults.push(...nextOps);
        requestEnvelope = {
          ...rubyRequest,
          control: {
            continue: true,
            // Ruby側再実行時に、実行結果を渡す
            ops: completedKvResults.map((op) => ({
              kind: op.kind,
              op: op.op,
              ok: op.ok,
              result: op.result,
