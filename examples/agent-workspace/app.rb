...
def workspace_request!(name, path, method: "GET", payload: nil)
  response, parsed, workspace = workspace_request(name, path, method: method, payload: payload)
...
get("/") do
  ...
  snapshot, = workspace_request!(workspace, "/state")
...
get("/api/workspaces/:name") do
  ...
  response, payload, = workspace_request(params["name"], "/state")
  status(response.status)
  payload.to_json
rescue WorkspaceShellError => e
  status(e.status)
