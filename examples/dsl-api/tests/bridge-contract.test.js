...
assert.ok(routesSource.includes('def auto_crud(app, model_class, prefix = nil)'), 'auto_crud helper should exist');
assert.ok(routesSource.includes('class Article < Homura::Model'), 'Article model should exist');
assert.ok(routesSource.includes('class Tag < Homura::Model'), 'Tag model should exist');
assert.ok(routesSource.includes('auto_crud($app, Article)'), 'Article CRUD should be auto-generated');
assert.ok(routesSource.includes('$app.get "/api/articles/published"'), 'published article endpoint should exist');
...
assert.ok(routesSource.includes('powered_by: "Homura::Model + mruby-metaprog"'), 'docs should describe the DSL');
