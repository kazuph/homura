...
- **儀式なしの非同期** — ビルド工程が、Workers の promise 形の binding 呼び出しを
  裏で吸収するので、`db.execute(...)` は coroutine ではなく
...
| [`sinatra-with-email`](examples/sinatra-with-email/) | <https://sinatra-with-email.kazu-san.workers.dev/> | Phase 17.5 auto-await デモ — `SEND_EMAIL` Cloudflare Email バインディング越しの POST `/send`。ユーザーコードは普通の同期的 Ruby の形を保つ。 |
...
| [`phlex-d1-todo`](examples/phlex-d1-todo/) | <https://phlex-d1-todo.kazu-san.workers.dev/> | Phlex 風 DSL と Literal 風 props で Ruby クラスとして UI を書き、`sequel-d1` 経由で D1 に永続化する TODO。 |
...
- **エッジでの非同期。** D1 / KV / fetch はすべて promise の形をしている。
  homura はビルドパイプライン側でその非同期境界を吸収するので、
  登録済みのメソッド（`db.execute`、`kv.get`、`Cloudflare::AI.run`、…）を
  呼ぶルート本体も同期的な見た目のまま保てる。
...
first-party Ruby だけを自動整形し、最終的な強制力は CI が持つ。
