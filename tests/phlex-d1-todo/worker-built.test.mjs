...
import worker from "../../examples/phlex-d1-todo/build/worker.entrypoint.mjs";

function createFakeD1() {
  const rows = [
    { id: 1, title: "Ship upstream Phlex", done: 0, created_at: 1 },
    { id: 2, title: "Keep Stimulus tiny", done: 1, created_at: 2 },
  ];
    ...
    prepare(sql) {
      const statement = {
        ...
        all() {
          ...
          return Promise.resolve({
            results: rows.slice().sort((a, b) => a.id - b.id),
          });
        },
        run() {
...
test("GET / renders upstream Phlex + Literal component HTML", async () => {
  const env = createEnv();
  ...
  assert.equal(text.startsWith('{"$$id"'), false);
  assert.match(text, /homura \+ Phlex \+ Literal \+ D1/);
  assert.match(text, /Ruby classes render this TODO app on Cloudflare Workers/);
  assert.match(text, /Ship upstream Phlex/);
  assert.match(text, /Keep Stimulus tiny/);
  assert.match(text, /button-primary/);
  assert.match(text, /button-danger/);
});

test("POST routes mutate the fake D1 binding and redirect back", async () => {
  const env = createEnv();
  ...
  const createResponse = await callWorker(
    "/todos",
    {
      ...
      body: new URLSearchParams({ title: "Use real Literal props" }),
    },
    env,
  );

  assert.equal(createResponse.status, 303);
  assert.equal(createResponse.headers.get("location"), "https://example.test/");
  assert.equal(env.DB.rows.at(-1).title, "Use real Literal props");

  const toggleResponse = await callWorker("/todos/1/toggle", { method: "POST" }, env);
  assert.equal(toggleResponse.status, 303);
  assert.equal(env.DB.rows.find((row) => row.id === 1).done, 1);

  const deleteResponse = await callWorker("/todos/2/delete", { method: "POST" }, env);
