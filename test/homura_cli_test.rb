...
Dir.mktmpdir do |dir|
  ...
  ok = assert("homura new scaffolds a project") do
    Dir.chdir(repo_root) do
      system("bundle", "exec", "ruby", cli, "new", app_dir) or
        raise("homura new failed")
    end

    raise "Gemfile missing" unless File.exist?(File.join(app_dir, "Gemfile"))

    config_ru = File.read(File.join(app_dir, "config.ru"))
    unless config_ru.include?("require_relative 'app/app'")
      raise "config.ru should require app/app relatively"
    end
    ...
    if config_ru.include?("run App")
      raise "config.ru should not emit `run App` (auto-registered now)"
    end

    app_rb = File.read(File.join(app_dir, "app", "app.rb"))
    if app_rb.include?("run App")
      raise "app/app.rb should not call run App directly"
    end

    unless app_rb.include?("require 'sinatra/base'")
      raise "app/app.rb should require sinatra/base"
    end

    if app_rb.include?("require 'sinatra/homura'")
      raise "app/app.rb should not require legacy sinatra/homura"
    end

    gemfile = File.read(File.join(app_dir, "Gemfile"))
    unless gemfile.include?("gem 'rake'")
      raise "Gemfile should include rake for generated tasks"
    end

    rakefile = File.read(File.join(app_dir, "Rakefile"))
    raise "Rake build task missing" unless rakefile.include?("task :build do")
    unless rakefile.include?("task dev: :build do")
      raise "Rake dev task missing"
    end

    unless rakefile.include?("task deploy: :build do")
      raise "Rake deploy task missing"
    end

    package = JSON.parse(File.read(File.join(app_dir, "package.json")))
    unless package.dig("scripts", "build") == "bundle exec rake build"
      raise "package build script should delegate to rake"
    end

    unless package.dig("scripts", "dev") == "bundle exec rake dev"
      raise "package dev script should delegate to rake"
    end

    unless package.dig("scripts", "deploy") == "bundle exec rake deploy"
      raise "package deploy script should delegate to rake"
    end
  end
...
Dir.mktmpdir do |dir|
  ...
  ok = assert("homura new --with-db scaffolds build task with db flag") do
    Dir.chdir(repo_root) do
      system("bundle", "exec", "ruby", cli, "new", app_dir, "--with-db") or
        raise("homura new --with-db failed")
    end

    gemfile = File.read(File.join(app_dir, "Gemfile"))
    unless gemfile.include?("gem 'sequel-d1'")
      raise "Gemfile should include sequel-d1"
    end

    rakefile = File.read(File.join(app_dir, "Rakefile"))
    unless rakefile.include?("'homura', 'build', '--standalone', '--with-db'")
      raise "Rake build task should pass --with-db"
    end

    unless rakefile.include?("task :compile do")
      raise "Rake db:migrate:compile task missing"
    end

    unless rakefile.include?("task local: :compile do")
      raise "Rake db:migrate:local task missing"
    end

    unless rakefile.include?("task remote: :compile do")
      raise "Rake db:migrate:remote task missing"
    end

    wrangler = File.read(File.join(app_dir, "wrangler.toml"))
    unless wrangler.include?("compatibility_flags = [\"nodejs_compat\"]")
      raise "wrangler.toml should include nodejs_compat"
    end

    unless wrangler.include?("[[d1_databases]]")
      raise "wrangler.toml should include D1 binding"
    end

    unless wrangler.include?("migrations_dir = \"db/migrate\"")
      raise "wrangler.toml should include migrations_dir"
    end

    migration = File.read(File.join(app_dir, "db", "migrate", "001_create_users.rb"))
    unless migration.include?("create_table?(:users)")
      raise "sample migration should create users table"
    end

    package = JSON.parse(File.read(File.join(app_dir, "package.json")))
    unless package.dig("scripts", "db:migrate:compile") == "bundle exec rake db:migrate:compile"
      raise "package migrate compile script missing"
    end

    unless package.dig("scripts", "db:migrate:local") == "bundle exec rake db:migrate:local"
      raise "package migrate local script missing"
    end

    unless package.dig("scripts", "db:migrate:remote") == "bundle exec rake db:migrate:remote"
      raise "package migrate remote script missing"
    end
  end
...
Dir.mktmpdir do |dir|
  ...
  ok = assert(
    "standalone build derives relative entrypoint imports for custom output paths"
  ) do
    ...
    unless entrypoint.include?("import \"./cf-runtime/setup-node-crypto.mjs\";")
      raise entrypoint
    end
    ...
    output, status = Open3.capture2e(
      ...
      "--input-type=module",
      "-",
      File.join(app_dir, "build", "worker.entrypoint.mjs"),
      stdin_data: node_script
    )
    raise output unless status.success?
    unless output.lines.map(&:strip).include?("200:custom-entrypoint-ok")
      raise output
    end
  end
...
Dir.mktmpdir do |dir|
  ...
  ok = assert(
    "standalone build uses config.ru without app/hello.rb and preserves redirect port"
  ) do
    output, status = Open3.capture2e(
      bundle_env,
      "bundle",
...
Dir.mktmpdir do |dir|
  ...
  ok = assert("standalone build falls back to app/app.rb without app/hello.rb") do
    ...
    raise output unless status.success?
    unless output.lines.map(&:strip).include?("200:ok-from-app-rb")
      raise output
    end
  end
...
Dir.mktmpdir do |dir|
  ...
  File.write(
    File.join(app_dir, "app", "hello.rb"),
    <<~RUBY
      # await: call
      # frozen_string_literal: true
      require 'app'
...
Dir.mktmpdir do |dir|
  ...
  File.write(
    File.join(app_dir, "views", "index.erb"),
    <<~ERB
      <li class="<%= 'completed' if @todos.first[:completed] %>"><%= @todos.first[:text] %></li>
    ERB
  )
  File.write(
    File.join(app_dir, "views", "layout.erb"),
    "<main><%= yield %></main>\n"
  )
  File.write(File.join(app_dir, "public", "robots.txt"), "User-agent: *\n")

  ok = assert(
    "standalone build auto-awaits sequel routes, coerces booleans, and applies default layout"
  ) do
    output, status = Open3.capture2e(
      bundle_env,
      "bundle",
    ...
    unless api_line == "API:200:{\"todos\":[{\"id\":1,\"text\":\"demo\",\"completed\":false}]}"
      raise output
    end
