...
ok = assert("resolves homura-runtime from loaded specs") do
  ...
  unless spec&.full_gem_path == "/tmp/new-runtime"
    raise "expected /tmp/new-runtime, got #{spec&.full_gem_path}"
  end
end
...
ok = assert("resolves homura-runtime lib path") do
  ...
  unless lib == "/tmp/new-runtime/lib"
    raise "expected /tmp/new-runtime/lib, got #{lib}"
  end
end
...
ok = assert("resolves sinatra-homura lib path") do
  ...
  unless lib == "/tmp/new-sinatra/lib"
    raise "expected /tmp/new-sinatra/lib, got #{lib}"
  end
end
...
ok = assert("resolves packaged homura-runtime vendor path") do
  Dir.mktmpdir do |dir|
    ...
    vendor = HomuraRuntime::BuildSupport.gem_vendor(
      runtime_name,
      loaded_specs: specs
    )
...
ok = assert("matches homura-runtime path dependency in Gemfile") do
  Dir.mktmpdir do |dir|
    ...
    unless vendor&.to_s == expected
      raise "expected #{expected}, got #{vendor}"
    end
  end
end
...
ok = assert("copies standalone runtime files into cf-runtime") do
  Dir.mktmpdir do |dir|
    ...
    File.write(
      File.join(runtime_root, "runtime", "setup-node-crypto.mjs"),
      "setup\n"
    )
    File.write(
      File.join(runtime_root, "runtime", "worker_module.mjs"),
      "worker\n"
    )

    target = HomuraRuntime::BuildSupport.ensure_standalone_runtime(
      dir,
      loaded_specs: specs
    )
    expected = File.join(dir, "build", "cf-runtime")
    raise "expected #{expected}, got #{target}" unless target.to_s == expected
    unless File.read(File.join(expected, "setup-node-crypto.mjs")) == "setup\n"
      raise "missing setup-node-crypto.mjs"
    end

    unless File.read(File.join(expected, "worker_module.mjs")) == "worker\n"
      raise "missing worker_module.mjs"
    end
  end
end
...
ok = assert(
  "standalone load paths prepend packaged sequel-d1 vendor before lib when with_db"
) do
  Dir.mktmpdir do |dir|
    ...
    [runtime_root, sinatra_root, sequel_root, app_root].each do |path|
    ...
    unless load_paths.index(sequel_vendor) < load_paths.index(sequel_lib)
      raise "vendor should precede lib: #{load_paths.inspect}"
    end
  end
end
...
ok = assert("standalone load paths do not require sinatra-homura") do
  Dir.mktmpdir do |dir|
    ...
    unless load_paths.include?(runtime_vendor)
      raise "missing #{runtime_vendor}"
    end
...
ok = assert("writes Opal gem prelude for supported Zeitwerk gems") do
  Dir.mktmpdir do |dir|
    ...
    unless body.include?("require \"phlex/fifo\"")
...
ok = assert("derives standalone namespaces from project name") do
  templates = HomuraRuntime::BuildSupport.standalone_namespace(
    "/tmp/demo-app",
    "Templates"
  )
  assets = HomuraRuntime::BuildSupport.standalone_namespace(
    "/tmp/demo-app",
    "Assets"
  )
  unless templates == "DemoAppTemplates"
    raise "expected DemoAppTemplates, got #{templates}"
  end

  unless assets == "DemoAppAssets"
    raise "expected DemoAppAssets, got #{assets}"
  end
end
...
ok = assert("prefixes standalone namespaces that would start with digits") do
  namespace = HomuraRuntime::BuildSupport.standalone_namespace(
    "/tmp/123-app",
    "Templates"
  )
  unless namespace == "App123AppTemplates"
    raise "expected App123AppTemplates, got #{namespace}"
  end
end
...
ok = assert("homura-runtime gemspec packages vendor shims") do
  ...
  unless spec.files.include?("vendor/digest.rb")
    raise "missing vendor/digest.rb"
  end

  unless spec.files.include?("vendor/cgi/escape.rb")
    raise "missing vendor/cgi/escape.rb"
  end

  unless spec.files.include?("vendor/rubygems/version.rb")
    raise "missing vendor/rubygems/version.rb"
  end

  raise "missing vendor/rack.rb" unless spec.files.include?("vendor/rack.rb")
  unless spec.files.include?("vendor/rack/request.rb")
    raise "missing vendor/rack/request.rb"
  end
end
...
ok = assert("sinatra-homura gemspec packages vendored sinatra sources") do
  ...
  unless spec.files.include?("vendor/sinatra/base.rb")
    raise "missing vendor/sinatra/base.rb"
  end
